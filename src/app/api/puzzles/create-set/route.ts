...
export async function GET(req: NextRequest) {
  ...
  console.log('[API GET /puzzles/create-set] Client identifier:', identifier);
  const { success, limit, remaining, reset } = await puzzleCreationLimiter.limit(identifier);

  if (!success) {
    console.warn('[API GET /puzzles/create-set] Rate limit exceeded:', { identifier, limit, remaining, reset });
